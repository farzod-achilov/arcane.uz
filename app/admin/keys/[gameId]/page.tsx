...
import {
  ...
  CheckCircle2, Archive, Settings, User,
} from 'lucide-react';
...
import { MOCK_GAME_STOCK, MOCK_KEY_RECORDS, MOCK_KEY_TRANSACTIONS } from '@/lib/admin/mockKeysData';
import type { ImportResult } from '@/lib/admin/adminKeysTypes';
...
export default function GameKeyManagerPage({ params }: { params: { gameId: string } }) {
  const game = MOCK_GAME_STOCK.find(g => g.gameId === params.gameId) ?? MOCK_GAME_STOCK[0];
  const keys = MOCK_KEY_RECORDS.filter(k => k.gameId === params.gameId);
  const txs  = MOCK_KEY_TRANSACTIONS;

  const [activeTab,    setActiveTab]    = useState<TabId>('overview');
  const [showImport,   setShowImport]   = useState(false);
  const [showAddKey,   setShowAddKey]   = useState(false);
  const [showMove,     setShowMove]     = useState(false);
  const [lowThreshold, setLowThreshold] = useState(String(game.lowStockThreshold));
  const [autoDisable,  setAutoDisable]  = useState(true);
  const [lastImport,   setLastImport]   = useState<ImportResult | null>(null);

  const totalAvail  = game.stockStore + game.stockDrop + game.stockBoth;
  const stockByType = { STORE: game.stockStore, DROP: game.stockDrop, BOTH: game.stockBoth };
  const alertCount  = (game.health === 'CRITICAL' || game.health === 'EMPTY') ? 1 : 0;

  const tabs: { id: TabId; label: string; icon: React.ElementType; badge?: number }[] = [
    { id: 'overview', label: 'Обзор',         icon: BarChart2       },
    { id: 'import',   label: 'Импорт',         icon: Upload          },
    { id: 'keys',     label: 'Ключи',          icon: List,            badge: keys.filter(k => k.status === 'AVAILABLE').length },
    { id: 'move',     label: 'Переместить',    icon: ArrowLeftRight  },
    { id: 'settings', label: 'Настройки',      icon: Settings,        badge: alertCount },
  ];
    ...
    <div className="p-6 space-y-5">
          ...
          <div className="flex-1 p-5">
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                <div className="flex items-center gap-2.5 mb-1 flex-wrap">
                  ...
                  <StockHealthBadge health={game.health} />
                  {!game.isActive && (
                ...
                <p className="font-body text-[#374151]" style={{ fontSize: '11px' }}>
                  ID: <span className="font-pixel text-[#4B5563]" style={{ fontSize: '10px' }}>{game.gameId}</span>
                </p>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-4 gap-4">
              {[
                ...
                { label: 'ПРОДАНО', value: game.sold,       color: '#7C3AED' },
              ].map(s => (
                <div key={s.label}>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeTab}
          ...
          {activeTab === 'overview' && (
            <div className="grid lg:grid-cols-3 gap-4">
              <div className="rounded-2xl p-5 space-y-2.5"
                ...
                <StatPill label="STORE ключей"  value={game.stockStore} color="#06B6D4" />
                <StatPill label="DROP ключей"   value={game.stockDrop}  color="#F59E0B" />
                <StatPill label="BOTH ключей"   value={game.stockBoth}  color="#9D60FA" />
                <div className="my-2" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }} />
                <StatPill label="Продано"   value={game.sold}     color="#7C3AED" />
                <StatPill label="Резерв"    value={game.reserved} color="#F59E0B" />
                <StatPill label="Отключено" value={game.disabled} color="#4B5563" />
                <div className="pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                  <p className="font-body text-[#374151] mb-1.5" style={{ fontSize: '10px' }}>
                    Запас: {totalAvail} / порог {game.lowStockThreshold}
                  </p>
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min(100, (totalAvail / Math.max(game.lowStockThreshold * 3, 1)) * 100)}%` }}
                      transition={{ duration: 0.7, ease: 'easeOut' }}
                      style={{
                        height: '100%', borderRadius: '4px',
                        background: game.health === 'OK' ? '#22C55E' : game.health === 'LOW' ? '#F59E0B' : game.health === 'CRITICAL' ? '#EF4444' : '#4B5563',
                      }}
                    />
                  </div>
                </div>
                    ...
                    return (
                      <motion.div key={tx.id}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {tx.username && (
                            <p className="font-body text-[#374151] mt-0.5" style={{ fontSize: '10px' }}>{tx.username}</p>
                          )}
                        </div>
          ...
          {activeTab === 'import' && (
            <div className="rounded-2xl p-6 space-y-5"
              ...
              <button onClick={() => setShowImport(true)}
                ...
                onMouseLeave={e => { (e.currentTarget.style.background = 'rgba(124,58,237,0.05)'); (e.currentTarget.style.borderColor = 'rgba(124,58,237,0.25)'); }}>
          ...
          {/* KEY LIST */}
          {activeTab === 'keys' && (
            <KeysTable keys={keys} onDisable={(ids) => console.log('Disable:', ids)} />
          )}
          ...
          {activeTab === 'move' && (
            <div className="rounded-2xl p-6"
              ...
              <div className="grid grid-cols-3 gap-3 mb-5">
                {[
                  { label: 'STORE', value: game.stockStore, color: '#06B6D4', desc: 'Магазин' },
                  { label: 'BOTH',  value: game.stockBoth,  color: '#9D60FA', desc: 'Магазин + Дропы' },
                  { label: 'DROP',  value: game.stockDrop,  color: '#F59E0B', desc: 'Дропы' },
                ].map(s => (
              ...
              <button onClick={() => setShowMove(true)}
                ...
                onMouseEnter={e => (e.currentTarget.style.background = 'rgba(157,96,250,1)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'rgba(157,96,250,0.85)')}>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl p-5"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button className="px-4 py-2 rounded-xl font-body ml-auto transition-all"
      ...
      <AnimatePresence>
        {showImport && (
          <ImportModal gameId={game.gameId} gameTitle={game.title}
            onClose={() => setShowImport(false)}
            onSuccess={r => { setLastImport(r); setShowImport(false); }} />
        )}
        {showAddKey && (
          <AddKeyModal gameId={game.gameId} gameTitle={game.title}
            onClose={() => setShowAddKey(false)} onSuccess={() => setShowAddKey(false)} />
        )}
        {showMove && (
          <MoveModal gameId={game.gameId} gameTitle={game.title} stockByType={stockByType}
            onClose={() => setShowMove(false)} onSuccess={() => setShowMove(false)} />
        )}
      </AnimatePresence>
    </div>
  );
