...
/* ─────────────────────────────────────────────────────────
   useG2aProducts — ⚠ UNVERIFIED integration, see lib/g2a/config.ts
───────────────────────────────────────────────────────── */
...
export interface UseProductsResult {
  ...
  isError: boolean;
  errorMsg: string;
  source: 'g2a' | 'mock' | 'mock_fallback' | null;
  total: number;
  refetch: () => void;
}

export function useG2aProducts(options: UseProductsOptions = {}): UseProductsResult {
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      const res = await globalThis.fetch(`/api/g2a/products?${params}`, { signal: ctrl.signal });
