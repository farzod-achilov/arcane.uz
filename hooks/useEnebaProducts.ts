...
/* ─────────────────────────────────────────────────────────
   useEnebaProducts
   Fetches product list from /api/eneba/products.
...
export interface UseProductsResult {
  ...
  isError: boolean;
  errorMsg: string;
  source: 'eneba' | 'mock' | 'mock_fallback' | null;
  total: number;
  refetch: () => void;
}

export function useEnebaProducts(options: UseProductsOptions = {}): UseProductsResult {
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      const res = await globalThis.fetch(`/api/eneba/products?${params}`, { signal: ctrl.signal });
