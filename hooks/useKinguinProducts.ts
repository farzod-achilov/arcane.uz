...
/* ─────────────────────────────────────────────────────────
   useKinguinProducts
   Fetches product list from /api/kinguin/products.
...
export interface UseProductsResult {
  ...
  isError: boolean;
  errorMsg: string;
  source: 'kinguin' | 'mock' | 'mock_fallback' | null;
  total: number;
  refetch: () => void;
}

export function useKinguinProducts(options: UseProductsOptions = {}): UseProductsResult {
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      const res = await globalThis.fetch(`/api/kinguin/products?${params}`, { signal: ctrl.signal });
