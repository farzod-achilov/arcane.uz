...
/* ─────────────────────────────────────────────────────────
   useDigisellerProducts
   Fetches product list from /api/digiseller/products.
   Falls back to mock data seamlessly on error.
───────────────────────────────────────────────────────── */
...
export interface UseProductsOptions {
  ...
  /** Skip auto-fetch (controlled mode) */
  skip?: boolean;
}
...
export interface UseProductsResult {
  ...
  isError: boolean;
  errorMsg: string;
  source: 'digiseller' | 'mock' | 'mock_fallback' | null;
  total: number;
  refetch: () => void;
}

export function useDigisellerProducts(options: UseProductsOptions = {}): UseProductsResult {
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      const res = await globalThis.fetch(
        `/api/digiseller/products?${params}`,
        { signal: ctrl.signal },
      );
